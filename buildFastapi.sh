#!/bin/bash
#
# Open-Automator FastAPI - Multi-platform Docker Build & Push Script
...
Build and optionally push Open-Automator FastAPI Docker image for multiple platforms.
...
ENVIRONMENT VARIABLES:
    ...
    IMAGE_NAME              Base image name (default: open-automator-fastapi)
    PLATFORMS               Target platforms
    DOCKERFILE              Dockerfile path (default: Dockerfile.fastapi)
...
TAGS CREATED:
    - USERNAME/open-automator-fastapi:1.0.0      (full version)
    - USERNAME/open-automator-fastapi:1.0        (major.minor)
    - USERNAME/open-automator-fastapi:1          (major)
    - USERNAME/open-automator-fastapi:latest     (latest)
...
# Print configuration
print_header "Open-Automator FastAPI Build"
...
BUILD_ARGS=(
    ...
    "--label" "org.opencontainers.image.title=Open-Automator API"
    "--label" "org.opencontainers.image.description=REST API for Open-Automator workflow execution"
