...
"""
Generates README.md and README.html from oa-*.py module documentation
With function list at the top of each module
"""
...
def extract_module_docs(file_path):
    """Extracts docstring from Python file"""
...
def parse_docstring(docstring):
    """Converts docstring to markdown format"""
...
def generate_markdown(modules_data):
    """Generates markdown from extracted data"""
    md = []


    md.append("# Open-Automator - Module Documentation\n")
    md.append("Automatic documentation of available modules and functions.\n")
    md.append("---\n")
    ...
    for module_data in modules_data:
        ...
        md.append(f"- [{module_name}](#{module_name.replace('-', '')}) - {func_count} functions")
    md.append("\n---\n")
    ...
    for module_data in modules_data:
        ...
        md.append(f"### 📋 Available functions ({len(functions)})\n")
    ...
    md.append("\n---\n")
    md.append("*Documentation automatically generated by `generate_readme.py`*\n")
...
def generate_html(modules_data):
    """Generates HTML from extracted data"""
    html = []
...
<head>
    ...
    <title>Open-Automator - Module Documentation</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <h1>🚀 Open-Automator - Module Documentation</h1>
        <p class="subtitle">Automatic documentation of available modules and functions.</p>
""")


    html.append('        <div class="module-index">')
    html.append('            <h2>📑 Module Index</h2>')
    html.append('            <ul>')
    ...
    for module_data in modules_data:
        ...
        html.append(f'                <li><a href="#{anchor}">{module_name}</a> - {func_count} functions</li>')
    ...
    for module_data in modules_data:
        ...
        html.append(f'            <h3>📋 Available functions ({len(functions)})</h3>')
        html.append('            <ul>')
        ...
        for func in functions:
            ...
            if parsed['returns']:
                ...
                html.append('                <strong>Returns:</strong>')
                for ret in parsed['returns']:
                    ret_clean = ret.strip()
    ...
    html.append('            <p>Documentation automatically generated by <code>generate_readme.py</code></p>')
    html.append('        </div>')
    html.append('    </div>')
    html.append('</body>')
    html.append('</html>')
...
def main():
    """Generates README.md and README.html from modules"""
    module_files = sorted(Path('./modules').glob('oa-*.py'))


    if not module_files:
        print("❌ No oa-*.py modules found")
        return
    ...
    for file_path in module_files:
        ...
        if data and data['functions']:
            ...
            print(f"   ✓ {len(data['functions'])} functions found")
    ...
    print(f"   {len(modules_data)} modules documented")
    total_functions = sum(len(m['functions']) for m in modules_data)
    print(f"   {total_functions} total functions")


    print(f"\n📖 View Markdown with: cat {output_md}")
    print(f"🌐 Open HTML in browser: {output_html}")
